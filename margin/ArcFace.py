...
class ArcMarginProduct(nn.Module):
    ...
    def __init__(self, in_features, out_features, s=10.0, m=0.4, easy_margin=True):
        ...
        # ms = np.array([0.2565609 , 0.19106605, 0.34785437, 0.34485614, 0.28542868,
        #                0.34959823, 0.28627682, 0.2748252 , 0.1522405 , 0.25235059])
        # ms = 0.3 - ms
        # self.cos_m = torch.tensor([math.cos(m) for m in ms]).cuda()
        # self.sin_m = torch.tensor([math.sin(m) for m in ms]).cuda()
