...
Rs = {'cifar_s1':54000, 'cifar_s2':50000, 'nuswide_21':5000, 'nuswide_81':5000, 'imagenet':1000}
...
def main():
    ...
    testloader, trainloader, databaseloader, R = datasets.create(args)
    num_classes = 10
    ...
    classifier = margin.create(name=args.classifier, feat_dim=args.feat_dim, 
                               num_classes=num_classes, scale=args.scale)

    hadamard = generate_hadamard_codebook(bit=args.feat_dim, num_classes=num_classes)
    np.savetxt(os.path.join(args.save_dir, 'hadamard.txt'), hadamard)
    hadamard = torch.from_numpy(hadamard).float().cuda()
    ...
    for epoch in range(args.max_epoch):
        ...
        if args.eval_freq > 0 and (epoch+1) % args.eval_freq == 0 or (epoch+1) == args.max_epoch:
            ...
            mAP_feat = get_mAP(code_and_labels['db_feats'], code_and_labels['db_labels'], 
                            code_and_labels['test_feats'], code_and_labels['test_labels'], Rs[args.dataset])
            mAP_sign = get_mAP(code_and_labels['db_codes'], code_and_labels['db_labels'], 
                            code_and_labels['test_codes'], code_and_labels['test_labels'], Rs[args.dataset])
